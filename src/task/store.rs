...
impl TaskStore {
    ...
    pub fn list_tasks_filtered(&self, query: &TaskListQuery) -> BabataResult<Vec<TaskRecord>> {
        ...
        bind_params.push(Value::from(i64::try_from(query.limit).map_err(|err| {
            BabataError::internal(format!(
                "Failed to convert task limit '{}' to i64: {}",
                query.limit, err
            ))
        })?));
        ...
        if let Some(offset) = query.offset {
            sql.push_str(" OFFSET ?");
            bind_params.push(Value::from(i64::try_from(offset).map_err(|err| {
                BabataError::internal(format!(
                    "Failed to convert task offset '{}' to i64: {}",
                    offset, err
                ))
            })?));
        }
